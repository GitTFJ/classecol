...
classify = function(text, classifier, sentiment){
  ...
  container = create_container(matrix, labels = NA, testSize = c(1:length(text)), virgin = F)

  #Predict outputs
  results = classify_models(container, mods[[1]])

  #Train the ensemble
  if(grepl("Nature", classifier)){
    ensem = predict(model[[2]], cbind(results[,c(TRUE,FALSE)], sm))
  } else {
